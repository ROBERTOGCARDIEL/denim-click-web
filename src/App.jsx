            ...
            <div style={{ display: 'grid', gap: 14, marginTop: 18 }}>
              ...
              {customer.delivery === 'Envíos' ? (
                <>
                  <select
                    style={styles.input}
                    value={customer.shipping_type || 'Domicilio'}
                    onChange={(e) =>
                      setCustomer((p) => ({
                        ...p,
                  ...
                  {customer.shipping_type === 'Domicilio' ? (
                    <>
                      <input
                        style={styles.input}
                        ...
                        onChange={(e) =>
                          setCustomer((p) => ({ ...p, recipient_name: e.target.value }))
                        }
                      />
                      ...
                      <input
                        ...
                        value={customer.recipient_phone || ''}
                        onChange={(e) =>
                          setCustomer((p) => ({ ...p, recipient_phone: e.target.value }))
                        }
                      />
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCustomer((p) => ({ ...p, postal_code: e.target.value }))
                        }
                      />
                    </>
                  ) : (
                    <input
                      ...
                      value={customer.ocurre_address || ''}
                      onChange={(e) =>
                        setCustomer((p) => ({ ...p, ocurre_address: e.target.value }))
                      }
                    />
                  )}
                </>
...
export default function App() {
  ...
  const sendOrder = async () => {
    ...
    if (customer.delivery === 'Envíos') {
      if (customer.shipping_type === 'Domicilio') {
        if (
          !customer.recipient_name?.trim() ||
          !customer.recipient_phone?.trim() ||
          !customer.address?.trim() ||
          !customer.postal_code?.trim()
        ) {
          alert('Completa los datos de envío a domicilio.')
          return
        }
      }

      if (customer.shipping_type === 'Ocurre') {
        if (!customer.ocurre_address?.trim()) {
          alert('Completa la dirección de ocurre.')
          return
        }
      }
    }
    ...
    try {
      ...
      const shippingExtra =
        customer.delivery === 'Envíos'
          ? customer.shipping_type === 'Domicilio'
            ? ` | Tipo de envío: Domicilio | Recibe: ${customer.recipient_name || '-'} | Contacto: ${customer.recipient_phone || '-'} | Dirección: ${customer.address || '-'} | CP: ${customer.postal_code || '-'}`
            : ` | Tipo de envío: Ocurre | Dirección de ocurre: ${customer.ocurre_address || '-'}`
          : ''

      const noteWithClient = specialClientSession?.active
        ? `${customer.notes || ''}${shippingExtra} | Cliente especial: ${specialClientSession.name} (${specialClientSession.client_tier})`
        : `${customer.notes || ''}${shippingExtra}`
      ...
      const shippingWhatsapp =
        customer.delivery === 'Envíos'
          ? customer.shipping_type === 'Domicilio'
            ? `%0A*Tipo de envío:* Domicilio%0A*Recibe:* ${customer.recipient_name || '-'}%0A*Contacto:* ${customer.recipient_phone || '-'}%0A*Dirección:* ${customer.address || '-'}%0A*Código postal:* ${customer.postal_code || '-'}`
            : `%0A*Tipo de envío:* Ocurre%0A*Dirección de ocurre:* ${customer.ocurre_address || '-'}`
          : ''
      ...
      const msg =
        ...
        `*Entrega:* ${customer.delivery || '-'}${shippingWhatsapp}%0A` +
